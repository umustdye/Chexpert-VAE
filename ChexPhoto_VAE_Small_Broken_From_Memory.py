...
def loss_fn(recon_x, x, mu, logvar, model):
    ...
    #mse = mse.detach()
    if model.gamma == None:
        model.gamma = loss.detach()
        
    else:
        model.gamma = .9*model.gamma + .1*loss.detach()
        #model.gamma = min(model.gamma, loss.detach()) #larger batch size
    #BCELoss = functional.binary_cross_entropy(recon_x, x, reduction="sum")
    KL_Div = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    
    model.kl_div = KL_Div
    model.loss_before = loss
    loss = loss/(2*model.gamma*model.gamma)
...
class CHEXPERT_Dataset(Dataset):
    ...
    def getAllItems(self):
        ...
        for img_path in self.photo_dir:
...
end_time = time()
print('Loading train dataset took %.3f' %(end_time - begin_time))
begin_time = time()
...
end_time = time()
print('Loading valid dataset took %.3f' %(end_time - begin_time))
...
print("LOSS STATS FOR .1 .9 MSE LOSS")
...
for t in range(epochs):
    ...
    for idx, (images) in enumerate(train_dataloader):
        ...
        running_kl += model.kl_div
        running_loss_before += model.loss_before
        running_gamma += model.gamma
        '''
        # print statistics
        running_loss += loss.item()
        if idx % 1000 == 999:    # print every 50000 mini-batches
    ...
    loss_graph.append(running_loss/num_examples)
    kl_div_graph.append(running_kl/num_examples)
    loss_before_graph.append(running_loss_before/num_examples)
    gamma_graph.append(running_gamma/num_examples)
    running_loss = 0.0
    running_kl = 0.0
    running_loss_before = 0.0
    running_gamma = 0.0
    ...
    if(t == 0):
        torch.save(model.state_dict(), "19model_0ep.pt")
    if(t == 5):
        torch.save(model.state_dict(), "19model_5ep.pt")
    if(t == 25):
        torch.save(model.state_dict(), "19model_25ep.pt")
    if(t == 50):
        torch.save(model.state_dict(), "19model_50ep.pt")
    if(t == 75):
        torch.save(model.state_dict(), "19model_75ep.pt")

print("Saving model...")
torch.save(model.state_dict(), "19model_100ep.pt")    
...
model.load_state_dict(torch.load("CHEXPERT_VAE_MODEL_SMALL_FULL_500EPOCHS.pt"))
#print(model)
model.eval()
...
with open('19loss_after.txt', 'w') as filehandle:
	for i in loss_graph:
		filehandle.write(str(i.item()) + '\n')
with open('19kl_div.txt', 'w') as filehandle:
        for i in kl_div_graph:
                filehandle.write(str(i.item()) + '\n')
with open('19gamma.txt', 'w') as filehandle:
        for i in gamma_graph:
                filehandle.write(str(i.item()) + '\n')
with open('19loss_before.txt', 'w') as filehandle:
        for i in loss_before_graph:
                filehandle.write(str(i.item()) + '\n')

#generate loss and kl_div graph
plt.title(".1 .9 Loss After KL Divervence")
plt.plot(loss_graph)
#plt.show()
plt.savefig("19loss_after.png")

plt.title(".1 .9 KL Divergence")
plt.plot(kl_div_graph)
#plt.show()
plt.savefig("19kl_div.png")

plt.title(".1 .9 Gamma")
plt.plot(gamma_graph)
#plt.show()
plt.savefig("19gamma.png")

plt.title(".1 .9 Loss Before KL Divergence")
plt.plot(loss_before_graph)
#plt.show()
plt.savefig("19loss_before.png")
